...
public class GuildCommands {
	...
	@Command(identifier = "guild claim",
			description = "Claim land for your guild",
			onlyPlayers = true)
		...
		new GuildGetter(gp) {
			...
			protected void onReturn(Guild returned) {
				...
				if (returned.hasTerritory(coord.getBlockAt())) {
					NoxGuildLocale.GUILD_ERROR_CLAIM_ALREADYOWNED.send(sender,
							returned);
					return;
				}
				...
				if (TerritoryUtils.isClaimed(coord)) {
					...
					NoxGuildLocale.GUILD_ERROR_CLAIM_ALREADYCLAIMED.send(sender,
							plot != null ? plot.getGuildOwner() : null);
					return;
				}
	...
	public void guildCreate(Player sender, @Arg(
		...
		NoxGuilds.getInstance().getChatChannel().broadcast(
				new FanceeMessage(NoxGuildLocale.GUILD_CREATED_BROADCAST.get(
						gp, newGuild)));
	}
	...
	public void guildJoin(
		...
		if (GuildUtil.isEnemyOf(guild, gp)) {
			NoxGuildLocale.PLAYER_ERROR_JOINGUILD_CURRENTLYENEMY.get(guild)
					.send(sender);
			return;
		}
		...
		guild.addMember(gp);
		guild.getChatChannel().broadcast(
				new FanceeMessage(NoxGuildLocale.GUILD_PLAYER_JOINED
						.get(gp)));
	}
	
	@Command(
			identifier = "guild kick",
			description = "Kicks a player from the guild",
			onlyPlayers = true)
	public void guildKick(final Player sender, @Arg(
			name = "player name",
			description = "The guild member to kick") final Player kicked) {
		...
		new GuildGetter(GuildPlayerManager.getInstance().getFromPlayer(sender)) {
			...
			public void onReturn(Guild returned) {
				...
				if (!returned.hasMember(kicked.getUniqueId())) {
					NoxGuildLocale.GUILD_ERROR_MEMBER_NOTFOUND.send(sender, kicked
							.getName());
					return;
				}
				
				returned.removeMember(GuildPlayerManager.getInstance()
						.getFromPlayer(kicked));
				
				returned.getChatChannel().broadcast(
						new FanceeMessage(NoxGuildLocale.GUILD_PLAYER_KICKED
								.get(kicked, returned)));
	...
	public void guildSetBadge(final Player sender) {
		...
		if ((held = sender.getItemInHand()) == null
			...
			NoxGuildLocale.ERROR_GENERIC.send(sender,
					"You must be holding an item first!");
			return;
		}
	...
	public void test(final Player sender) {
		new FanceeMessage(new FancyMessage("")
				.then("[")
